...
def Round_odd(eng, x0, x1, x2, x3, k, i, c0):
    temp_key = eng.allocate_qureg(16)
    Round_constant_XOR(eng, x0, i)
    x1 = logical_left(eng, x1, 1)
    with Compute(eng):
        CNOT16(eng, k, temp_key)
        CNOT16(eng, x1, temp_key)
    x1 = logical_right(eng, x1, 1)
    improved_adder(eng, temp_key, x0, c0, 15)
    #reverse
    x1 = logical_left(eng, x1, 1)
    Uncompute(eng)
    # All(Measure) | temp_key
    # print_hex(eng, temp_key)
    x1 = logical_right(eng, x1, 1)
